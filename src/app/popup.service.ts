...
export class PopupService {
  ...
  constructor(public popoverController: PopoverController, public dataService: GameDataService) { }
   ...
   async newMessage() {
      let random: number = Math.floor(Math.random() * this.quotes.length); 
       return `"${this.quotes[random].text}" - ${this.quotes[random].author}`; 


      /*
      await fetch("https://web.archive.org/web/20210101051009/https://type.fit/api/quotes", { mode: 'no-cors'})
    ...
    // ************ NOTICE ****************
    // Our quotes api went down (https://type.fit/api/quotes) a week before this project was finished which we discovered moments before
    // a presentation so this is a temporary bandaid fix, please see the GitHub Issue for more information. 
    quotes = [
      {
